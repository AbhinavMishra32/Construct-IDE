...
async function createAdaptiveFrontierHarness(options: {
  ...
  const planJob = service.createPlanningPlanJob({
    ...
    answers: questionSession.session.questions.map((question) => ({
      ...
      optionId: deterministicCreationOptionId(question.id)
    }))
  });
...
test("ConstructAgentService creates question and plan jobs and persists the resulting state", async () => {
  ...
  try {
    ...
    assert.equal(questionSession.session.questions.length, 2);
    assert.equal(loggedStages.some((stage) => stage.startsWith("research-")), false);
    assert.ok(loggedStages.includes("creation-intake"));
  ...
  try {
    ...
    assert.ok(
      loggedStages.some((stage) =>
        stage.includes("creation-intake:Preparing build controls complete")
      )
    );
    assert.equal(loggedStages.some((stage) => stage.includes("research-")), false);
...
test("ConstructAgentService narrows intro learner files that pack too many build moves into one task surface", async () => {
  ...
  try {
    ...
    const step = blueprint.steps?.find((entry) => entry.id === "step.utility");

    assert.ok(step);
    const listedMoves = Array.from(step.doc.matchAll(/^\d+\.\s/gm)).length;
    assert.ok(listedMoves <= 4, `expected at most 4 listed build moves, received ${listedMoves}`);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("ConstructAgentService remaps a single targeted repair patch back onto the only failing file", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "construct-agent-blueprint-remap-repair-"));
  let planCalls = 0;
  let blueprintCalls = 0;
  let fullRepairCalls = 0;
  let patchRepairCalls = 0;
  ...
  const validBundle = {
    projectName: "tiny-server",
    projectSlug: "tiny-server",
    description: "A tiny TypeScript project with one server entrypoint.",
    language: "typescript",
    entrypoints: ["src/server.ts"],
    supportFiles: [
      {
    ...
    hiddenTests: [
      {
        path: "tests/server.test.ts",
        content: "import { startServer } from '../src/server';\n\ntest('returns the starter string', () => {\n  expect(startServer()).toBe('ok');\n});\n"
      }
    ],
    steps: [
      {
        id: "step.start-server",
        title: "Start the tiny server",
        summary: "Implement the first runtime function.",
        doc: "Edit `src/server.ts` at the `TASK:start-server` anchor and implement `startServer()`.",
        lessonSlides: [
          markdownSlide("## Start with one visible behavior\n\nA single function is enough to make the project concrete.")
        ],
        anchor: {
          file: "src/server.ts",
          marker: "TASK:start-server",
        ...
        tests: ["tests/server.test.ts"],
        concepts: ["typescript.functions", "domain.servers"],
        constraints: ["Keep the first step intentionally tiny."],
        checks: [],
        estimatedMinutes: 15,
        difficulty: "intro"
      }
    ...
    dependencyGraph: {
      nodes: [{ id: "component.server", label: "Server", kind: "component" }],
      edges: []
    },
    tags: ["typescript", "server"]
  };
  ...
  const service = new ConstructAgentService(root, {
    ...
    search: {
      async research(query) {
        return {
          query,
          answer: "Start with one visible runtime behavior.",
          sources: []
        };
      }
    },
    projectInstaller: {
      async install() {
        return {
          status: "skipped",
          packageManager: "none",
          detail: "No install needed for the regression."
        };
      }
    },
    llm: {
      async parse({ schemaName, schema, prompt }) {
        ...
        if (schemaName === "construct_goal_scope") {
          return schema.parse({
            ...
            recommendedMinSteps: 1,
            recommendedMaxSteps: 2,
            rationale: "The project is intentionally tiny."
          });
        }
          ...
          return schema.parse({
            ...
            questions: [
                ...
                options: [
                  {
                    id: "solid",
                    label: "Comfortable",
                    description: "I can work with small typed functions confidently.",
                    confidenceSignal: "comfortable"
                  },
                  {
                    id: "partial",
                    label: "Need guidance",
                    description: "I can follow examples, but I still want help while implementing.",
                    confidenceSignal: "shaky"
                  },
                  {
                    id: "new",
              ...
              },
              {
                conceptId: "domain.servers",
                category: "domain",
                prompt: "How familiar are you with a tiny runtime entrypoint?",
                options: [
                  {
                    id: "solid",
                    label: "I know the idea well",
                    description: "I understand what a small entrypoint function is doing.",
                    confidenceSignal: "comfortable"
                  },
                  {
                    id: "partial",
                    label: "I know the idea but want examples",
                    description: "I understand the concept, but I still want a walkthrough.",
                    confidenceSignal: "shaky"
                  },
                  {
                    id: "new",
        ...
        if (schemaName === "construct_generated_project_plan") {
          planCalls += 1;
          return schema.parse({
            summary: "Teach one runtime behavior and implement the first server function.",
            knowledgeGraph: {
              concepts: [
                {
                  id: "typescript.functions",
                  ...
                  labelPath: ["TypeScript", "Functions"],
                  confidence: "shaky",
                  rationale: "The learner asked for help with tiny functions."
                }
              ],
              strengths: [],
              gaps: ["Runtime entrypoints"]
            },
            architecture: [
              {
                id: "component.server",
                ...
                kind: "component",
                summary: "Exposes the first visible runtime function.",
                dependsOn: []
              }
            ],
            steps: [
              {
                ...
                title: "Start the tiny server",
                kind: "implementation",
                objective: "Implement the first runtime function.",
                rationale: "Start with one visible behavior.",
                concepts: ["typescript.functions", "domain.servers"],
                dependsOn: [],
                validationFocus: ["returns the starter string"],
                suggestedFiles: ["src/server.ts"],
                implementationNotes: ["Keep it intentionally tiny."],
                quizFocus: ["Can explain what the function returns."],
                hiddenValidationFocus: ["starter string returned"]
              }
            ],
            suggestedFirstStepId: "step.start-server"
        ...
        if (schemaName === "construct_generated_blueprint_file_patch") {
          ...
          return schema.parse({
            ...
            canonicalFiles: [
              {
                path: "src/rateLimiter/TokenBucketRateLimiter.ts",
                content: "export function startServer(): string {\n  return 'ok';\n}\n"
              }
            ],
            learnerFiles: [],
        ...
        if (schemaName === "construct_authored_blueprint_step") {
          return schema.parse({
            summary: "Teach the first runtime function and implement the starter string.",
            doc: "Edit `src/server.ts` and implement `startServer()`.",
            lessonSlides: [
              markdownSlide("## Start with one visible runtime behavior\n\nThe first step only needs one concrete function."),
              markdownSlide("## Why this stays intentionally tiny\n\nA single return value is enough to anchor the runtime mental model before the project grows.")
            ],
            checks: []
          });
        }
  ...
  const service = new ConstructAgentService(root, {
    ...
    llm: {
      async parse({ schemaName, schema }) {
        ...
        if (schemaName === "construct_authored_blueprint_step") {
          return schema.parse({
            summary: "Teach one focused utility function.",
            doc: "Implement the utility function in `src/index.ts`.",
            lessonSlides: [
              markdownSlide("## One build move at a time\n\nKeep the first slice local."),
              markdownSlide("## Reuse it next\n\nThe next step can build on this exact function.")
            ],
            checks: []
          });
        }
...
async function expectBlueprintGenerationToComplete(
  root: string,
  input: {
    bundle: ReturnType<typeof buildBlueprintGuardBundle>;
  }
): Promise<ConstructAgentService> {
  let tick = 0;
...
function deterministicCreationOptionId(questionId: string): string {
  switch (questionId) {
    case "question.artifact-identity":
      return "build-technology";
    case "question.artifact-first-slice":
      return "first-working-behavior";
    case "question.teaching-depth":
      return "concise-build-notes";
    default:
      throw new Error(`No deterministic test answer for planning question ${questionId}`);
  }
}
...
test("goal-scope fallback keeps build requests implementation-first instead of recasting them as learning goals", async () => {
  ...
  try {
    ...
    assert.equal(scope.artifactShape, "next + nextjs artifact");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
...
test("goal-scope fallback does not treat from scratch alone as beginner hand-holding", async () => {
  ...
  try {
    ...
    assert.equal(scope.engagementMode, "implementation-first");
    assert.equal(scope.recommendedMinSteps, 2);
...
test("goal-scope analysis is deterministic and preserves ambiguous framework-only artifact locks", async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), "construct-agent-goal-scope-artifact-lock-"));

  try {
    const service = new ConstructAgentService(root, {
      llm: {
        async parse({ schemaName }) {
          throw new Error(`Unexpected schema request: ${schemaName}`);
        }
      },
      logger: {
    ...
    const artifactLock = resolveArtifactLock("implement reactjs from scratch in typescript");
    const scope = await (service as unknown as {
      determineGoalScope(
        goal: string,
    ...
    assert.equal(artifactLock.needsClarification, true);
    assert.deepEqual(artifactLock.requestedTechnology, ["react", "reactjs"]);
    assert.deepEqual(artifactLock.artifactNouns, []);
    assert.equal(scope.engagementMode, "implementation-first");
    assert.equal(scope.shouldResearch, false);
    assert.equal(scope.recommendedQuestionCount, 2);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("planning question generation uses deterministic artifact-lock intake", async () => {
  ...
  try {
    ...
    const result = await waitForJobCompletion(service, questionJob.jobId);
    const payload = result.result as {
      session: {
        detectedLanguage: string;
        detectedDomain: string;
        questions: Array<{ conceptId: string; prompt: string; options: Array<{ id: string }> }>;
      };
    };

    assert.equal(payload.session.detectedLanguage, "typescript");
    assert.equal(payload.session.questions.length, 2);
    assert.equal(payload.session.questions[0]?.conceptId, "artifact.identity");
    assert.match(payload.session.questions[0]?.prompt ?? "", /which artifact/i);
    assert.deepEqual(
      payload.session.questions[0]?.options.map((option) => option.id),
      ["build-technology", "build-with-technology", "smallest-faithful-artifact"]
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
  ...
  try {
    ...
    const planJob = service.createPlanningPlanJob({
      ...
      answers: questionSession.session.questions.map((question, index) => ({
        questionId: question.id,
        answerType: "option" as const,
        optionId: index === 0 ? "build-technology" : "concise-build-notes"
      }))
    });
    ...
    assert.equal(parsedPlanPrompt.creationContract.architecture, "artifact-first-project-compiler");
    assert.equal(parsedPlanPrompt.creationContract.stagePolicy.intake, "deterministic");
    assert.equal(parsedPlanPrompt.creationContract.stagePolicy.scope, "deterministic");
    assert.equal(parsedPlanPrompt.creationContract.stagePolicy.research, "disabled-by-default");
    ...
    assert.equal(parsedBlueprintPrompt.creationContract.architecture, "artifact-first-project-compiler");
    assert.equal(
      parsedBlueprintPrompt.creationContract.hardRules.some((rule) =>
        /may not replace the requested artifact/i.test(rule)
      ),
      true
    );
  } finally {
