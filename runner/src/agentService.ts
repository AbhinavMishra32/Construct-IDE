...
function validateGeneratedLearnerStepBudget(input: {
  ...
  if (taskGapRegions.length > maxTaskRegions) {
    throw new Error(
      `${input.context} ${difficultyLabel.toLowerCase()} step ${input.step.id} exposes ${taskGapRegions.length} learner task gaps in learnerFiles file ${anchorFile}. ${difficultyLabel} steps must keep only ${maxTaskRegions} active unfinished region${maxTaskRegions === 1 ? "" : "s"} visible at a time.`
    );
  }

  if (constructionUnits.length > maxBuildMoves) {
    throw new Error(
      `${input.context} ${difficultyLabel.toLowerCase()} step ${input.step.id} packs ${constructionUnits.length} build moves into learnerFiles file ${anchorFile}. ${difficultyLabel} steps should stay within ${maxBuildMoves} build moves so the learner only faces the next concrete slice.`
    );
  }

  if (responsibilityKinds.size > maxResponsibilityKinds) {
    throw new Error(
      `${input.context} ${difficultyLabel.toLowerCase()} step ${input.step.id} mixes ${responsibilityKinds.size} construction responsibility kinds in learnerFiles file ${anchorFile}. ${difficultyLabel} steps should stay within ${maxResponsibilityKinds} responsibility kinds and one main unknown at a time.`
    );
  }
}
...
function looksLikeLearnerTaskGapLine(line: string): boolean {
  if (/Build sequence for this step/i.test(line)) {
    return false;
  }

  if (/Keep this slice local/i.test(line)) {
    return false;
  }
...
const LEARNER_TASK_REGION_SIGNATURE_PATTERN =
  /^(?:export\s+)?(?:default\s+)?(?:async\s+)?function\b|^(?:public|private|protected|static|readonly|\s)*(?:async\s+)?[A-Za-z_$][\w$]*\s*\([^;{)]*\)\s*(?::\s*[^=]+)?\s*\{|^\s*constructor\s*\(/i;
...
function buildLessonAuthoringBrief(
  ...
  const requiredCoverage = [
    ...
    "What exact artifact the learner will implement in this step",
    "How the concept behaves in code or data",
    "The concrete input/output or before/after shape the learner should expect",
    "Definitions for any named abstractions or jargon before they are reused",
    "A worked example or conceptual code sketch that is structurally similar to the exercise",
...
function inferGoalScopeFallback(goal: string): GoalScope {
  ...
  if (mentionsComplexScope || wordCount >= 10) {
    return {
      ...
      recommendedMinSteps: mentionsBeginnerScope ? 6 : 5,
      recommendedMaxSteps: mentionsBeginnerScope ? 12 : 10,
  ...
  return {
    ...
    recommendedMinSteps: mentionsBeginnerScope ? 5 : 3,
    recommendedMaxSteps: mentionsBeginnerScope ? 9 : 6,
...
function buildPlanGenerationInstructions(): string {
  return [
    ...
    "For algorithms, utilities, libraries, and class-based artifacts, the first implementation step must be the naive version of the real artifact: the smallest honest behavior a learner can fully understand.",
    "Grow complexity by showing a concrete shortcoming of the previous version, then adding the smallest new concept that fixes that shortcoming.",
    "Do not start with the final production shell and one TODO hole. Avoid private-field-heavy classes, validation helpers, injected clocks, async queues, maps, generic adapters, or full config objects in step 1 unless the user explicitly asked for production hardening first.",
    "For limiters specifically, start with a tiny bucket/counter behavior first, then introduce capacity, time leakage, validation, clocks, async acquire, and edge cases in separate later slices as examples demand them.",
    ...
    "For beginner-facing implementation steps, one step should usually own one main unknown: for example validation, one helper, one branch, one calculation, or one return shape, not all of them at once.",
...
function buildBlueprintGenerationInstructions(): string {
  return [
    ...
    "Use intro difficulty for the first naive slice of algorithm, utility, parser, limiter, or library API work unless the user explicitly asked to start from a production-grade version.",
    "Never use a polished production-shaped file with many unexplained fields or helpers as the learner's first view of a small algorithm. Keep the first file boring, inspectable, and intentionally incomplete at one focused behavior.",
    "When later steps add production structure, each added field, helper, method, or type must be motivated by a failing example or visible limitation from the previous simpler version.",
    ...
    "If the learner is new, avoid first steps that require validation, async control flow, state mutation, return-shape design, and algorithm math all in the same task surface.",
    ...
    "For implementation-first steps, teach the exact artifact the learner is about to build, not just surrounding theory. If the learner will implement a function, class, reducer branch, parser node, or UI boundary, explicitly teach that artifact's role, inputs, outputs, and invariants.",
    "Before the exercise, show at least one near-transfer example that is structurally similar to the task the learner is about to do. The example should demonstrate the same kind of transformation, API shape, or data flow without dumping the final solution.",
    "Do not stop at abstract explanation. Translate the concept into the exact code move the learner will make next: what changes, what stays fixed, and why that implementation shape is correct.",
    "For implementation-first steps, one of the last slides should explicitly include sections equivalent to: what you will implement, success looks like, and what is out of scope in this step.",
    "The exercise should feel like the immediate next inch after the lesson, not like a distant transfer problem. Prefer near-transfer teaching before far-transfer exercises.",
    ...
    "doc should also make the step boundary concrete: what the learner must implement now, what success looks like in observable behavior, and what is explicitly not required yet.",
...
function extractGeneratedBlueprintFileValidationTarget(
  ...
  const oversizedTaskSurfaceMatch = message.match(
    /(intro|core) step [^ ]+ (?:exposes \d+ learner task gaps|packs \d+ build moves|mixes \d+ construction responsibility kinds) in learnerFiles file ([^.\n]+)\./i
  );

  if (oversizedTaskSurfaceMatch) {
    return {
      kind: "oversized-task-surface",
      group: "learnerFiles",
      path: normalizePathValue(oversizedTaskSurfaceMatch[2] ?? ""),
      error: message
    };
  }
...
function collectGeneratedBlueprintFileValidationTargets(
  ...
  try {
    validateGeneratedBlueprintDraftIntegrity(draft, context);
  } catch (error) {
    const target = extractGeneratedBlueprintFileValidationTarget(
      error instanceof Error ? error.message : String(error)
    );
...
function buildAdaptiveFrontierGenerationInstructions(): string {
  return [
    ...
    "For algorithm, parser, utility, limiter, or library work, preserve the naive-first progression when regenerating a frontier. Do not jump from a simple request into a production-shaped class with many unexplained fields and one TODO.",
    "If the current frontier is too production-shaped, rewrite it back to the smallest honest behavior first, then let later frontier steps introduce one new concept at a time.",
...
function buildLessonAuthoringInstructions(context: {
  ...
  return [
    ...
    "For the first step of an algorithm, utility, limiter, parser, or library, teach the naive version first. Explain why this deliberately small version exists and what limitation will force the next improvement.",
    "Do not teach a production-shaped class before the learner has earned each part of it. If the current code includes a field, method, helper, type, constructor argument, or invariant, the chapter must explain exactly why that piece exists before the handoff.",
    "When a later step introduces a more serious design, tie it to a concrete failing example from the previous version: for example too many requests, no time awareness, no capacity clamp, non-deterministic tests, or no async waiting.",
    "Do not preview later production concepts in enough detail that they feel like current requirements. Name them only as future pressure when that helps the learner understand the next local move.",
    ...
    "The chapter should explicitly remove hidden leaps: translate the explanation into the exact implementation move, API usage, data transformation, or state update the learner must make.",
    "For implementation-first steps, one of the final slides should include sections equivalent to: what you will implement, success looks like, and what is not required in this step yet.",
