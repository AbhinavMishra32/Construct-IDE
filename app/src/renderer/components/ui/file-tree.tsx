...
): T => (typeof updater === "function" ? (updater as (value: T) => T)(currentValue) : updater)
...
const Tree = forwardRef<HTMLDivElement, TreeViewProps>(
  (
    ...
    const expandedItems = expandedItemsProp ?? uncontrolledExpandedItems

    const setExpandedItems = useCallback(
      (updater: SetStateAction<string[] | undefined>) => {
        if (expandedItemsProp !== undefined) {
          const nextValue = resolveStateAction(expandedItemsProp, updater) ?? []
          onExpandedItemsChange?.(nextValue)
          return
        }

        setUncontrolledExpandedItems((current) => {
          const nextValue = resolveStateAction(current, updater)
          onExpandedItemsChange?.(nextValue ?? [])
          return nextValue
        })
      },
    ...
    useEffect(() => {
      const targetedId = selectedIdProp ?? initialSelectedId

      if (targetedId) {
        expandSpecificTargetedElements(elements, targetedId)
      }
    }, [elements, expandSpecificTargetedElements, initialSelectedId, selectedIdProp])
        ...
        <div className={cn("size-full", className)}>
          ...
          >
            <AccordionPrimitive.Root
              ...
              type="multiple"
              value={expandedItems}
              className="construct-file-tree-root flex flex-col gap-1"
              dir={dir as Direction}
            >
              {treeChildren}
            </AccordionPrimitive.Root>
    ...
    return (
      ...
      >
        <AccordionPrimitive.Trigger
          className={cn(
            "construct-file-tree-folder-trigger flex items-center gap-1 rounded-md text-sm",
            className,
            {
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? icon ?? <FolderOpenIcon className="size-4" />)
        ...
        <AccordionPrimitive.Content className="construct-file-tree-content data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down relative h-full overflow-hidden text-sm">
          ...
          <AccordionPrimitive.Root
            dir={direction}
            type="multiple"
            className="construct-file-tree-branch ml-5 flex flex-col gap-1 py-1 rtl:mr-5"
            value={expandedItems}
          >
            {children}
          </AccordionPrimitive.Root>
  ...
  (
    ...
    return (
      <button
        ...
        disabled={!isSelectable}
        className={cn(
          "construct-file-tree-file flex w-full items-center gap-1 rounded-md pr-1 text-sm duration-200 ease-in-out rtl:pr-0 rtl:pl-1",
          {
      ...
      >
        ...
        <span className="construct-file-tree-label">{children}</span>
      </button>
    )
  }
)
