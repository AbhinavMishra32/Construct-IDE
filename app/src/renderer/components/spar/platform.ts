/**
 * Platform-dependent copy. Credentials go to whatever secret store the app binds
 ...
 */
/* Read from the attribute `main.tsx` stamps on <html> from the same probe the
   chrome tokens use, so the copy below and the window-control insets can never
   disagree about which OS this is. */
const platform = document.documentElement.dataset.opalineOs ?? "darwin";
