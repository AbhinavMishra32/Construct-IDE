...
export function ConceptSummaryCard({
  ...
  const className = [
    "group flex w-full min-w-0 flex-col gap-2 rounded-[8px] border border-border/70 bg-card/82 text-left text-foreground shadow-sm transition-[background-color,border-color,box-shadow] duration-150",
    variant === "chat" ? "p-3 hover:border-border hover:bg-card" : "p-2.5 hover:border-border hover:bg-muted/20",
    ...
    <>
      <span className="flex min-w-0 items-start justify-between gap-3">
        <span className="flex min-w-0 items-start gap-2.5">
          <span className="mt-0.5 grid size-8 shrink-0 place-items-center rounded-[8px] border bg-background text-muted-foreground">
          ...
          <span className="min-w-0">
            ...
            <strong className="block truncate text-sm font-semibold">{concept.title}</strong>
          </span>
        </span>
        {onOpen ? (
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground transition-colors group-hover:text-foreground">
            {actionLabel}
