 ...
 *  the rest fills in a concept Construct has not met before. */
...
export class LocalStore {
  ...
  constructor(path: string) {
    ...
    /* Where a challenge came from, as one JSON column rather than eight. Null for
       everything Construct wrote, which is every row that existed before this. */
  ...
  /* The transcript syncs. Everything else the cloud holds is what Construct concluded;
  ...
  listNotices(limit=6,trackId?:string|null):ConstructNotice[]{const scope=this.learningTrackId(trackId);if(!scope)return[];return (this.db.prepare("SELECT id,title,body,created_at FROM learner_notices WHERE dismissed_at IS NULL AND track_id=? ORDER BY created_at DESC LIMIT ?").all(scope,limit) as Array<Record<string,unknown>>).map((row)=>({id:String(row.id),title:String(row.title),body:String(row.body),createdAt:String(row.created_at)}));}
  ratingHistory(trackId?:string|null):RatingPoint[]{const scope=this.learningTrackId(trackId);if(!scope)return[];return (this.db.prepare("SELECT * FROM rating_points WHERE track_id=? ORDER BY occurred_at").all(scope) as Array<Record<string,unknown>>).map((row)=>({id:String(row.id),rating:Number(row.rating),provisional:Boolean(row.provisional),reason:String(row.reason),occurredAt:String(row.occurred_at)}));}
  learnerProgress(trackId?:string|null):LearnerProgress{const scope=this.learningTrackId(trackId);const history=this.ratingHistory(scope);const rating=history.at(-1)??this.ensureRating(scope);return{rating,ratingHistory:history.length?history:[rating],abilities:this.abilityStates(scope),patterns:this.listPatterns(scope),notices:this.listNotices(6,scope)};}
  ...
  todayRecommendation():TodayRecommendation|null{
    ...
    const source=question?.source?.source??"construct";
  ...
  /** The concept for a slug, created if Construct has not met it. The agent is allowed
  ...
  private backfillTracks(){const sessions=this.db.prepare("SELECT id,title,original_goal,created_at,updated_at FROM sessions WHERE track_id IS NULL AND context<>'baseline' ORDER BY updated_at DESC").all() as Array<{id:string;title:string;original_goal:string;created_at:string;updated_at:string}>;if(!sessions.length)return;
    ...
    const existing=this.activeTrack();const trackId=existing?.id??randomUUID();if(!existing){const newest=sessions[0]!;const oldest=sessions.at(-1)!;const title=sessions.length===1?newest.title.slice(0,80):"General practice";const goal=sessions.length===1?newest.original_goal:"Continue adaptive practice across my existing Construct history.";this.db.prepare("INSERT INTO tracks (id,title,goal,status,emphasis,priorities,investigating,monitoring,created_at,updated_at) VALUES (?,?,?,'active','[]','[]','[]','[]',?,?)").run(trackId,title,goal,oldest.created_at,newest.updated_at);}
  ...
  private ensureRating(trackId?:string|null):RatingPoint{const scope=this.learningTrackId(trackId);const current=this.ratingHistory(scope).at(-1);if(current)return current;const point={id:randomUUID(),rating:1200,provisional:true,reason:"Initial provisional rating",occurredAt:new Date().toISOString()} satisfies RatingPoint;if(scope)this.db.prepare("INSERT INTO rating_points (id,rating,provisional,reason,occurred_at,track_id) VALUES (?,?,?,?,?,?)").run(point.id,point.rating,1,point.reason,point.occurredAt,scope);return point;}
  private recalculateRating(reason:string,trackId?:string|null){const scope=this.learningTrackId(trackId);const states=this.abilityStates(scope);const prior=this.ratingHistory(scope).at(-1)??this.ensureRating(scope);if(!states.length)return prior;const weight=states.reduce((sum,item)=>sum+Math.max(0.15,item.confidence),0);const performance=states.reduce((sum,item)=>sum+item.proficiency*Math.max(0.15,item.confidence),0)/weight;const rating=Math.round(700+performance*1100);const evidence=states.reduce((sum,item)=>sum+item.evidenceCount,0);if(Math.abs(rating-prior.rating)<10&&prior.provisional===(evidence<8))return prior;const point={id:randomUUID(),rating,provisional:evidence<8,reason,occurredAt:new Date().toISOString()} satisfies RatingPoint;if(scope)this.db.prepare("INSERT INTO rating_points (id,rating,provisional,reason,occurred_at,track_id) VALUES (?,?,?,?,?,?)").run(point.id,point.rating,point.provisional?1:0,point.reason,point.occurredAt,scope);return point;}
 ...
 *  every challenge Construct wrote itself has none — and a row written by a build that
 *  stored a shape this one no longer understands reads as "Construct wrote it" rather
