...
const tools = {
  ...
  "record-concept": hostTool(
    ...
    [
      ...
      "This is the learner's own knowledge base: they will reopen these notes later, so write the explanation for them to read, not for you to remember.",
      "Call it when you introduce an idea, see evidence of understanding, or find a gap.",
      "Levels: 0 unseen, 1 recognises pieces, 2 guided understanding, 3 practice ready, 4 applies reliably, 5 transfers and teaches.",
      "A scoped task is only fair at level 3 or above.",
      "Send summary, why, example and docs the first time you introduce a concept. On later calls you may send only the level and note; anything you leave out keeps what is already written.",
    ].join(" "),
    z.object({
      ...
      summary: z.string().max(1_200).optional().describe("What the idea is, in two or three sentences the learner can reread"),
      why: z.string().max(800).optional().describe("Why it matters — what goes wrong without it"),
      example: z.string().max(2_000).optional().describe("A short worked example, ideally from this learner's own project. Markdown code fences are fine"),
