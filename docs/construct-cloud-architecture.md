...
Construct Cloud is the hosted-compute path. When a user chooses Construct Cloud in AI settings, the desktop app sends OpenAI-compatible model calls to a hosted backend instead of using local API keys. That backend owns provider keys, account login, subscriptions, and usage accounting.
...
- Construct Cloud base URL.
- A desktop bearer token minted after account login.
- Which Construct Cloud model to request.
